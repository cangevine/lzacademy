class Ability
  ...
  def initialize(user)
    ...
    if user.role?(:admin)
      ...
      can :create, [Student, Registration, EmergencyForm, NotificationAddress]
      ...
      can :manage, NotificationAddress do |na|
        na.try(:registration).try(:student).try(:parent).try(:id) == user.id
      end
